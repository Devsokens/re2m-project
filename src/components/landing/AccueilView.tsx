...
export const AccueilView: React.FC<AccueilViewProps> = ({ onStartDemo }) => {
  const stats = [
  ...
  // Real clients from website
  const partners = [
    { name: 'COLAS Gabon', label: 'Infrastructures' },
    { name: 'SEEG', label: 'Énergie & Eau' },
    { name: 'SGEPP', label: 'Hydrocarbures' },
    { name: 'PERENCO', label: 'Pétrole & Gaz' },
    { name: 'CDC', label: 'Caisse des Dépôts' }
  ];
    ...
    <div className="space-y-16 animate-fadeIn bg-white text-[#0f172a]">
      ...
      <section className="bg-[#002366] text-white py-12 text-center rounded-3xl max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 shadow-xl">
        <div className="max-w-2xl mx-auto space-y-4">
          <h3 className="font-serif text-2xl font-bold">Cabinet Connecté & PWA</h3>
          <p className="text-xs text-slate-300">
            En complément de nos services d'audit, explorez notre système de cartes membres dématérialisées virtuelles.
          </p>
          <button
            onClick={onStartDemo}
            className="bg-white text-[#002366] font-bold px-6 py-2.5 rounded-xl hover:bg-slate-100 transition-colors text-xs"
          >
