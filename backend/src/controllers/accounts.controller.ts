...
export const createAccount = async (req: Request, res: Response) => {
  ...
  // If the admin sets a password themselves, use it and skip returning a
  // temp password (they already know it) — otherwise generate one.
  const wasAutoGenerated = !input.password;
  ...
  res.status(201).json({ ...toApiAccount(data), tempPassword: wasAutoGenerated ? password : null });
};
