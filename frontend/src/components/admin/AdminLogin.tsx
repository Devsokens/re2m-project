...
import { ShieldCheck, Key, Mail, Lock, BarChart3, Users, FileCheck2, Loader2 } from 'lucide-react';
import confetti from 'canvas-confetti';
import { apiClient, authToken, ApiError } from '../../lib/apiClient';
...
export const AdminLogin: React.FC<AdminLoginProps> = ({ onLoginSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { token, user } = await apiClient.post<{ token: string; user: AuthenticatedAdmin }>('/api/auth/login', {
        email,
        password
      });
      authToken.set(token);
      ...
      setError(err instanceof ApiError ? err.message : 'Impossible de contacter le serveur. Réessayez plus tard.');
    } finally {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 text-slate-800 text-xs rounded-xl pl-9 pr-3 py-3 border border-slate-200 focus:border-[#002366] focus:bg-white focus:outline-none"
                />
              </div>
            </div>
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full bg-[#002366] hover:bg-blue-900 text-white font-bold py-3.5 rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2 text-xs cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
              >
