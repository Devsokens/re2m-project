...
export const GestionCMS: React.FC<GestionCMSProps> = ({
  ...
  useEffect(() => {
    let active = true;
    cmsStorage.getDraftLayout(selectedPage).then((blocks) => {
      if (active) setDraftBlocks(blocks);
    });
    return () => {
      active = false;
    };
  ...
  const handleUpdateBlocks = (updated: CMSBlock[]) => {
    ...
    cmsStorage.saveDraftLayout(selectedPage, updated).catch((err) => console.error('Échec de l\'enregistrement du brouillon :', err));
  ...
  const handlePublish = () => {
    cmsStorage
      ...
      .catch((err) => console.error('Échec de la publication :', err));
  };

  const handleResetToDefault = () => {
    // Not implemented server-side yet — the factory-default content now lives
    // in backend/sql/seed.sql rather than in this file (see Phase 2 roadmap).
    window.alert("La réinitialisation aux valeurs d'usine sera réactivée dans une prochaine mise à jour.");
  };
